-- TASK-CR-0104-T: canonical cancellation RPC for marketplace and shop orders.
-- Does not modify create_marketplace_order, pay_with_balance or 0056 functions.
...
BEGIN
  IF to_regprocedure('public.cancel_order(uuid)') IS NOT NULL THEN
    RAISE EXCEPTION '0057 expected public.cancel_order(uuid) to be absent';
  END IF;
  IF to_regclass('public.orders') IS NULL
    ...
    RAISE EXCEPTION '0057 required order/escrow/listing tables are missing';
  END IF;
  IF to_regprocedure('public.release_shop_order_stock(uuid,uuid,integer)') IS NULL THEN
    RAISE EXCEPTION '0057 requires public.release_shop_order_stock(uuid,uuid,integer)';
  END IF;
END;
$$;
...
BEGIN
  ...
  IF v_order.status IN ('paid','shipped','delivered','refund_requested','disputed')
     ...
     OR EXISTS (
       ...
       WHERE po.business_type = 'order' AND po.business_id = p_order_id AND po.status = 'paid'
     ) THEN
    RETURN jsonb_build_object('success', false, 'code', 'REFUND_REQUIRED',
      'error', 'Paid or escrowed orders must use the refund flow', 'status', v_order.status);
  END IF;
  ...
  WHERE business_type = 'order' AND business_id = p_order_id AND status = 'pending';
  ...
  IF v_order.product_id IS NOT NULL THEN
    ...
    IF COALESCE((v_stock_result->>'success')::BOOLEAN, FALSE) IS NOT TRUE THEN
      RAISE EXCEPTION '0057 shop stock release failed for order %', p_order_id;
    END IF;
  END IF;
  ...
  IF v_order.listing_id IS NOT NULL AND to_regclass('public.market_listings') IS NOT NULL THEN
    EXECUTE 'UPDATE public.market_listings SET status = ''active'', updated_at = NOW() WHERE id = $1 AND status IN (''pending'',''reserved'')'
    USING v_order.listing_id;
  END IF;

  IF v_order.seller_id IS NOT NULL THEN
    INSERT INTO public.notifications(user_id, type, title, content, related_id)
    VALUES(v_order.seller_id, 'order_cancelled', 'Order cancelled',
      'Order ' || v_order.order_no || ' was cancelled', p_order_id);
  END IF;
...
COMMENT ON FUNCTION public.cancel_order(UUID) IS
  'CR-0104-T: party-authorized cancellation for unpaid orders; paid/frozen orders are routed to refund.';
